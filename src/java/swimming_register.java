 ...
 */
public class swimming_register extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           String pass = (String)session.getAttribute("pass");
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet swimming_register</title>");            
            out.println("</head>");
            out.println("<body>");
             try
            {
                ...
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/sports","root","");
                      ...
                      PreparedStatement ps =con.prepareStatement("select * from swimming");
                     out.println(date +"<br>");
               ...
               while(rs.next())
               {
                   ...
                   out.println("<html><body><script>alert('Date and Time has already fixed by other user!!!');window.location.assign('swimming_register.html');</script></body></html>");
                        
                     }
                     else
                     {
                  
                         PreparedStatement ps1 =con.prepareStatement("insert into swimming values(?,?,?,?,?,?,?,?,?,?,?)");
