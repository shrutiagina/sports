...
public class update_stud extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet update_stud</title>");            
            out.println("</head>");
            out.println("<body>");
                ...
                try
            {
               ...
               PreparedStatement ps = con.prepareStatement("update stud_create set NAME=?,EMAIL=?, Prev_SCIENCE_MARKS=?, Prev_MATHS_Marks=?,Prev_COMPUTER_MARKS=?, Prev_GK_MARKS=?  WHERE REGISTER_NO=?");
               ps.setString(1,name);
               ps.setString(2,mail);
               ps.setInt(3, scien);
               ps.setInt(4, mat);
            ...
            out.println("<h1>Servlet update_stud at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
